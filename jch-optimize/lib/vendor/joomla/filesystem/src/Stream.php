...
class Stream
{
     ...
     * @since   1.0
     */
    public static function getStream($usePrefix = \true, $ua = null, $uamask = \false)
    {
    ...
    public function open($filename, $mode = 'r', $useIncludePath = \false, $context = null, $usePrefix = \false, $relative = \false, $detectprocessingmode = \false)
    {
        ...
        if (!$this->fh) {
            ...
            if ($error === null || $error['message'] === '') {
                ...
                $error = array('message' => \sprintf('Unknown error opening file %s', $filename));
            }
    ...
    public function close()
    {
        ...
        if (!$res) {
            ...
            if ($error === null || $error['message'] === '') {
                // Error but nothing from php? Create our own
                $error = array('message' => 'Unable to close stream');
            }
            throw new FilesystemException($error['message']);
        }
        // Reset this
    ...
    public function filesize()
    {
        ...
        if (!$res) {
            ...
            if ($error === null || $error['message'] === '') {
                // Error but nothing from php? Create our own
                $error = array('message' => 'Failed to get file size. This may not work for all streams.');
            }
            throw new FilesystemException($error['message']);
        }
        $this->filesize = $res;
    ...
    public function gets($length = 0)
    {
        ...
        if (!$res) {
            ...
            if ($error === null || $error['message'] === '') {
                // Error but nothing from php? Create our own
                $error = array('message' => 'Unable to read from stream');
            }
            throw new FilesystemException($error['message']);
        }
        // Return the result
    ...
    public function seek($offset, $whence = \SEEK_SET)
    {
        ...
        if ($res == -1) {
            ...
            if ($error === null || $error['message'] === '') {
                // Error but nothing from php? Create our own
                $error = array('message' => 'Unable to seek in stream');
            }
            throw new FilesystemException($error['message']);
        }
        // Return the result
    ...
    public function tell()
    {
        ...
        if ($res === \false) {
            ...
            if ($error === null || $error['message'] === '') {
                // Error but nothing from php? Create our own
                $error = array('message' => 'Unable to determine the current position in stream');
            }
            throw new FilesystemException($error['message']);
        }
        // Return the result
    ...
    public function write(&$string, $length = 0, $chunk = 0)
    {
        ...
        do {
            ...
            if ($res === \false) {
                ...
                if ($error === null || $error['message'] === '') {
                    // Error but nothing from php? Create our own
                    $error = array('message' => 'Unable to write to stream');
                }
                throw new FilesystemException($error['message']);
            }
            if ($res === 0) {
    ...
    public function chmod($filename = '', $mode = 0)
    {
        ...
        if ($res === \false) {
            ...
            if ($error === null || $error['message'] === '') {
                // Error but nothing from php? Create our own
                $error = array('message' => 'Unable to change mode of stream');
            }
            throw new FilesystemException($error['message']);
        }
        // Return the result
    ...
    public function applyContextToStream()
    {
        ...
        if ($this->fh) {
            ...
            if (!$retval) {
                ...
                if ($error === null || $error['message'] === '') {
                    // Error but nothing from php? Create our own
                    $error = array('message' => 'Unable to apply context to stream');
                }
                throw new FilesystemException($error['message']);
            }
        }
    ...
    public function removeFilter(&$resource, $byindex = \false)
    {
        ...
        if (!$res) {
            ...
            if ($error === null || $error['message'] === '') {
                // Error but nothing from php? Create our own
                $error = array('message' => 'Unable to remove filter from stream');
            }
            throw new FilesystemException($error['message']);
        }
        return $res;
     ...
     * @throws  FilesystemException
     */
    public function copy($src, $dest, $context = null, $usePrefix = \true, $relative = \false)
    {
        // Capture PHP errors
        if (\PHP_VERSION_ID < 70000) {
            // @Todo Remove this path, when PHP5 support is dropped.
     ...
     * @throws  FilesystemException
     */
    public function move($src, $dest, $context = null, $usePrefix = \true, $relative = \false)
    {
        // Capture PHP errors
        if (\PHP_VERSION_ID < 70000) {
            // @Todo Remove this path, when PHP5 support is dropped.
        ...
        if (!$res) {
            ...
            if ($error === null || $error['message'] === '') {
                // Error but nothing from php? Create our own
                $error = array('message' => 'Unable to move stream');
            }
            throw new FilesystemException($error['message']);
        }
        $this->chmod($dest);
     ...
     * @throws  FilesystemException
     */
    public function delete($filename, $context = null, $usePrefix = \true, $relative = \false)
    {
        // Capture PHP errors
        if (\PHP_VERSION_ID < 70000) {
            // @Todo Remove this path, when PHP5 support is dropped.
        ...
        if (!$res) {
            ...
            if ($error === null || $error['message'] === '') {
                // Error but nothing from php? Create our own
                $error = array('message' => 'Unable to delete stream');
            }
            throw new FilesystemException($error['message']);
        }
        return $res;
     ...
     * @throws  FilesystemException
     */
    public function upload($src, $dest, $context = null, $usePrefix = \true, $relative = \false)
    {
